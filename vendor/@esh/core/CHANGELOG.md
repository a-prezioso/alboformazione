...
- Fix: i sotto-path `@esh/core/<component>` (es. `@esh/core/button`, `@esh/core/card`) ora funzionano correttamente per i consumer JavaScript.

  In precedenza Vite (con `preserveModules` + tree-shaking) eliminava i file barrel `dist/components/<name>/index.js` perché contenevano solo re-export. Risultato: gli export sotto-path dichiarati in `package.json` puntavano a file inesistenti — l'`import { EshCard } from '@esh/core/card'` falliva a runtime con `Cannot find module`.

  Soluzione: post-build script (`scripts/emit-component-stubs.mjs`) che emette automaticamente uno stub `index.js` per ogni cartella componente in cui Vite non lo ha generato. Lo stub re-esporta dai file `.js` sibling. Idempotente, side-effect zero per le build dei consumer esistenti.

  Nessun cambio per chi importava già via root (`import '@esh/core'`).

### Note

- Bump allineato col gruppo `linked` (tokens/core/icons/react/presentation) per release coerente del DS.
...
- Scaffold del pacchetto con Lit + TypeScript + Vite
- `<esh-button>` con variants primary/secondary/tertiary/ghost/danger, size sm/md/lg
- `<esh-input>` con label, hint, validation, types text/email/password/number
- `<esh-card>` con variants elevated/outlined/filled, modalità interactive
- Export granulare per tree-shaking (`@esh/core/button`, `@esh/core/input`, `@esh/core/card`)
